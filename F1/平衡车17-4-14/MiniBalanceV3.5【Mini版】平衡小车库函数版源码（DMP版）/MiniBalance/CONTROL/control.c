...
{  
   float Bias;
	 ...
	 sum_error += Bias;
	 balance=bal_kp*Bias+ Gyro*bal_kd+ bal_ki*sum_error;   //===计算平衡控制的电机PWM  PD控制   kp是P系数 kd是D系数 
	 return balance;
}
...
//		OLED_ShowFloat(0,2,Encoder_Integral,8,16);
		Velocity=Encoder_Least*kp+Encoder_Integral*ki;                          //===速度控制	
...
{
			if(angle<-25||angle>25||1==Flag_Stop)//电池电压低于11.1V关闭电机
			{	                                                 //===倾角大于40度关闭电机
				Moto1 = Moto2 =0;
      }
      return 0;			
