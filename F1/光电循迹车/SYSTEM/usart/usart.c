...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)  set_r=atoi(&SET_buf[0]);
	{
		...
		{
			Res[i]='\0';
			if(Res[0]=='a') pid_r.Proportion=atoi(&Res[1]);
			if(Res[0]=='b') pid_l.Proportion=atoi(&Res[1]); 
			if(Res[0]=='c') pid_r.Derivative=atoi(&Res[1]); 
			if(Res[0]=='d') pid_l.Derivative=atoi(&Res[1]);
			if(Res[0]=='e') pid_r.Integral=atoi(&Res[1]); 
			if(Res[0]=='f') pid_l.Integral=atoi(&Res[1]);  
			if(Res[0]=='g') set_x=(float)atoi(&Res[1])/10; 
			i=-1;
		}
		i++;
  } 
