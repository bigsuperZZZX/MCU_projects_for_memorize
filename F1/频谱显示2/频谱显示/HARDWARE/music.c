...
void TIM3_IRQHandler()	 //定时器3中断函数
{
	...
	for(i=0;i<128;i++)
	{
		...
		Fft_Real[LIST_TAB[i]] = (int)( ADC_GetConversionValue(ADC1)*2 );//返回最近一次 ADCx 规则组的转换结果
		PAout(0)=1;
	}
	for(i=0;i<128;i++)
  {
	...
	for(i=0;i<9;i++)
  {
    Height[i] = ( Fft_Image[i*5+61]+Fft_Image[i*5+62]+Fft_Image[i*5+63]+Fft_Image[i*5+64]+Fft_Image[i*5+65]+Fft_Image[i*5+66]+Fft_Image[i*5+67] )/3;
		if(Height[i]>63)
			Height[i]=63;
		if(Height[i]<0)
			Height[i]=0;
...
void light_init(void)
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2|GPIO_Pin_3|GPIO_Pin_6|GPIO_Pin_8|GPIO_Pin_9|GPIO_Pin_13|GPIO_Pin_11|GPIO_Pin_12|GPIO_Pin_13|GPIO_Pin_14|GPIO_Pin_15;	 
...
void light_show(char led,char act)
{
	if(act==1)
	{
		switch(led)
		{
			...
			case 19:L71_ON();break;
			case 20:L72_ON();break;
			case 21:L73_ON();break;
			case 22:L81_ON();break;
			case 23:L82_ON();break;
			case 24:L83_ON();break;
		}
	}
	if(act==0)
	{
		switch(led)
		{
			...
			case 19:L71_OFF();break;
			case 20:L72_OFF();break;
			case 21:L73_OFF();break;
			case 22:L81_OFF();break;
			case 23:L82_OFF();break;
			case 24:L83_OFF();break;
		}
	}
}
