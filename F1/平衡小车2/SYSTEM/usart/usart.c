...
void USART1_IRQHandler(void)                	//串口1中断服务程序
{
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)  set_r=atoi(&SET_buf[0]);
	{
		...
		{
			Res[i]='\0';
			if(Res[0]=='a') Flag_Left=(atoi(&Res[1]));
			if(Res[0]=='b') Flag_Right=(atoi(&Res[1])); 
			if(Res[0]=='c') Flag_Qian=(atoi(&Res[1])); 
			if(Res[0]=='d') Flag_Hou=atoi(&Res[1]); 
			if(Res[0]=='e') turn_Kd=(float)atoi(&Res[1])/10; 
			i=-1;
			if(Res[0]=='e')
			{
				OLED_ShowFloat(0,0,Flag_Left,6,16);
				OLED_ShowFloat(64,0,Flag_Right,6,16);
				OLED_ShowFloat(0,2,Flag_Qian,6,16);
				OLED_ShowFloat(0,4,Flag_Hou,6,16);
				OLED_ShowFloat(64,4,turn_Kd,6,16);
			}
		}
		i++;
  } */
