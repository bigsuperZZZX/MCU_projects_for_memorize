...
u8 TP_Read_XY2(u16 *x,u16 *y) 
{
    ...
    &&((y2<=y1&&y1<y2+ERR_RANGE)||(y1<=y2&&y2<y1+ERR_RANGE)) && (x1+x2)!=0)
    {
        *x=((x1+x2)/2)*tp_dev.xfac+tp_dev.xoff;
        *y=((y1+y2)/2)*tp_dev.yfac+tp_dev.yoff;
        return 1;
    }else return 0;	  
}  
//////////////////////////////////////////////////////////////////////////////////		  
...
{	  
	POINT_COLOR=RED;
	LCD_ShowString(40,160,lcddev.width,lcddev.height,16,"x1:",1);
 	LCD_ShowString(40+80,160,lcddev.width,lcddev.height,16,"y1:",1);
 	LCD_ShowString(40,180,lcddev.width,lcddev.height,16,"x2:",1);
 	LCD_ShowString(40+80,180,lcddev.width,lcddev.height,16,"y2:",1);
	LCD_ShowString(40,200,lcddev.width,lcddev.height,16,"x3:",1);
 	LCD_ShowString(40+80,200,lcddev.width,lcddev.height,16,"y3:",1);
	LCD_ShowString(40,220,lcddev.width,lcddev.height,16,"x4:",1);
 	LCD_ShowString(40+80,220,lcddev.width,lcddev.height,16,"y4:",1);  
 	LCD_ShowString(40,240,lcddev.width,lcddev.height,16,"fac is:",1);     
...
{								 
	...
	LCD_Clear(WHITE);//清屏 	   
	POINT_COLOR=BLACK;
	LCD_ShowString(40,40,160,100,16,(char*)TP_REMIND_MSG_TBL,1);//显示提示信息
				...
				case 4:	 //全部四个点已经得到
					...
					if(abs(tp_dev.xfac)>2||abs(tp_dev.yfac)>2)//触屏和预设的相反了.
					{
						cnt=0;
 				    	TP_Drow_Touch_Point(lcddev.width-20,lcddev.height-20,WHITE);	//清除点4
   	 					TP_Drow_Touch_Point(20,20,RED);								//画点1
						LCD_ShowString(40,26,lcddev.width,lcddev.height,16,"TP Need readjust!",1);
					...
					POINT_COLOR=BLUE;
					LCD_Clear(WHITE);//清屏
					LCD_ShowString(35,110,lcddev.width,lcddev.height,16,"Touch Screen Adjust OK!",1);//校正完成
					delay_ms(1000);
					TP_Save_Adjdata();  
 					LCD_Clear(WHITE);//清屏   
					return;//校正完成				 
...
u8 TP_Init(void)
{
  GPIO_InitTypeDef  GPIO_InitStructure;	
	...
	//	AT24CXX_Init();		//初始化24CXX
		...
		tp_dev.xfac = -0.0628140718;
		tp_dev.yfac = 0.0904977396;
		tp_dev.xoff = 0x00F9;
		tp_dev.yoff = 0xFFE9;
