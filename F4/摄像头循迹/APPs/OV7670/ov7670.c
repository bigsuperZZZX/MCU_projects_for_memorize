...
void Line_Follow(void)
{
	int i;
	u32 sum_x_base;
	u8 count=0,cols_count=0,x_base=0;
	for(i=37599;i>37439;i--)
	{
		if(out[i]>30)
		{
			sum_x_base+=i%160;
	...
	x_base=(u8)sum_x_base/count;
	LCD_Fast_DrawPoint(x_base*2,i/160,RED);
	count=0;
	sum_x_base=0;
/*	for(i=37439;i>=800;i--)
	{
		if(out[i]>20&&(i%160>(x_base-30))&&(i%160<(x_base+30)))
		{
			sum_x_base+=i%160;
			count++;
		}
...
void roberts(u8 in[38400],u8 out[38400],int rows,int cols)   //边缘检测函数
{
 ...
 void camera_convert_display(void)   //边缘检测完整读取+显示，怕忘记所以单独搞一个，YUV输出
{
	u16 Gray;
	u32 j;
// 	u16 color;
