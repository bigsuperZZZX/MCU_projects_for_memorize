...
{ 
	...
	LCD_ShowxNum(0,0,(u32)sizeof(long),4,12,0);	
	FSMC_SRAM_Init();
	LCD_ShowxNum(0,50,(u32)buffer,32,12,1);
	//for(j=0;j<3000;j++)
		for(i=5;i<160*240;i++)
		{
			*(buffer+i) = i;
			LCD_ShowxNum(0,12,*(buffer+i),5,12,0);
			LCD_ShowxNum(0,30,*(buffer+i-5),5,12,0);	
			delay_ms(100);
		}
	LCD_ShowxNum(0,12,(u32)same_flag,4,12,0);	
	rtp_test();	
	delay_ms(1500);	
